 ...
 */
class auth_plugin_mnetsync extends auth_plugin_base {

    /**
     * Constructor.
     */
    function auth_plugin_mnetsync() {
        $this->authtype = 'mnetsync';
        $this->config = get_config('auth_mnetsync');
    ...
    function user_authorise($token, $useragent) {
        ...
        if ($courses = $DB->get_records_sql($sql, array($user->id, $remoteclient->id))) {
            foreach($courses as $course) {
                $userdata['myhosts'][] = array('name'=> $course->hostname, 'url' => $CFG->wwwroot.'/auth/mnetsync/jump.php?hostid='.$course->hostid, 'count' => $course->count);
            }
        }

        $userdata['customfields'] = $this->get_custom_user_fields($userid);
        return $userdata;
    }
    ...
    function start_jump_session($mnethostid, $wantsurl, $wantsurlbackhere=false) {
        ...
        if (empty($this->config->rpc_negotiation_timeout)) {
            ...
            set_config('rpc_negotiation_timeout', '30', 'auth_mnetsync');
        }

        // get the host info
        $mnet_peer = new mnet_peer();
        $mnet_peer->set_id($mnethostid);
        $mnet_peer->set_applicationid($this->get_mnetsync_appid());
    ...
    function confirm_mnet_session($token, $remotepeer) {
        ...
        $mnetrequest->set_method('auth/mnetsync/auth.php/user_authorise');
        ...
        $customfields = $remoteuser->customfields ? $remoteuser->customfields : '';
        $remoteuser = mnet_strip_user($remoteuser, mnet_fields_to_import($remotepeer));

        $remoteuser->auth = 'mnetsync';
        ...
        foreach ((array) $remoteuser as $key => $val) {
            ...
            if ($key == '_mnet_userpicture_timemodified' and empty($CFG->disableuserimages) and isset($remoteuser->picture)) {
                ...
                if (!empty($val) and $localtimemodified < $val) {
                    ...
                    $fetchrequest->set_method('auth/mnetsync/auth.php/fetch_user_image');
                    ...
                    if ($fetchrequest->send($remotepeer) === true) {
                        if (strlen($fetchrequest->response['f1']) > 0) {
                            ...
                            if ($newrev = process_icon($usercontext, 'user', 'icon', 0, $imagefilename)) {
        ...
        if (!$firsttime) {
            ...
            // set up the RPC request
            $mnetrequest = new mnet_xmlrpc_client();
            $mnetrequest->set_method('auth/mnetsync/auth.php/update_enrolments');
        ...
        $this->save_custom_user_fields($localuser->id, $customfields);
    ...
    function process_config($config) {
        ...
        set_config('auto_add_remote_users',   $config->auto_add_remote_users,   'auth_mnetsync');
        */

        // save settings
        set_config('rpc_negotiation_timeout', $config->rpc_negotiation_timeout, 'auth_mnetsync');
    ...
    function keepalive_client() {
        ...
        foreach($usersArray as $mnethostid => $users) {
            ...
            $mnet_request->set_method('auth/mnetsync/auth.php/keepalive_server');
            ...
            $mnet_request = new mnet_xmlrpc_client();
            $mnet_request->set_method('auth/mnetsync/auth.php/refresh_log');
    ...
    function kill_parent($username, $useragent) {
        ...
        if (false != $mnetsessions) {
            ...
            $mnet_request = new mnet_xmlrpc_client();
            $mnet_request->set_method('auth/mnetsync/auth.php/kill_children');
    ...
    function kill_children($username, $useragent) {
        ...
        foreach($mnetsessions as $mnetsession) {
            ...
            $mnet_request = new mnet_xmlrpc_client();
            $mnet_request->set_method('auth/mnetsync/auth.php/kill_child');
    ...
    /**
     * [get_custom_user_fields description]
     * @param  [type] $userid [description]
     * @return [type]         [description]
     */
    private function get_custom_user_fields($userid) {
        global $DB;
        ...
        return serialize($DB->get_records_sql_menu($sql, array('userid' => $userid)));
    }

    /**
     * Strips custom user fields from the user object that don't exist locally.
     * @param  array  $customfields [description]
     * @return array  [description]
     */
    private function strip_custom_user_fields(array $customfields) {
        global $DB;

        $fieldnames = $DB->get_fieldset_select('user_info_field', 'shortname', '1');
        $fields = array();
    ...
    /**
     * [save_custom_user_fields description]
     * @param int    $userid [description]
     * @param string $fields [description]
     */
    private function save_custom_user_fields($userid, $fields) {
        global $DB;
        ...
        $strippedfields = $this->strip_custom_user_fields($fields);
        foreach ($strippedfields as $fieldname => $fieldvalue) {
            $fieldid = $DB->get_field('user_info_field', 'id', array('shortname' => $fieldname));

            if ($oldrecord = $DB->get_record('user_info_data', array('userid' => $userid, 'fieldid' => $fieldid))) {
                $oldrecord->data = $fieldvalue;
                $DB->update_record('user_info_data', $oldrecord);
            } else {
                $newrecord = array(
                        'userid'  => $userid,
                        'fieldid' => $fieldid,
    ...
    /**
     * Retrieves the mnetsync application ID from the database
     * @param  string $name application name (optional, defaults to 'moodle_cfs').
     * @return int          application ID
     */
    private function get_mnetsync_appid($name = 'moodle_cfs') {
        global $DB;
        return $DB->get_field('mnet_application', 'id', array('name' => $name));
    }
}
